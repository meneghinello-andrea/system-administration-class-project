...
Mentre negli incidenti \english{hardware} ritroviamo per esempio:
...
Il supporto iniziale offerto è determinato dalla tipologia di richiesta che l'utente/cliente richiede al \english{Service Desk}. 

La \keyword{classificazione} determina perciò il supporto che l'utente/cliente richiede. Deve chiaramente definire come la funzione di \english{Service Desk} risponde alla richiesta e non chi, fisicamente, risponde.

Per questo motivo la convenzione consigliata ed utilizzata dal \english{framework} \ac{Information-Technology-Infrastructure-Library} imposta come primo elemento della tassonomia il \keyword{tipo}. Il tipo descrive un più ampio coinvolgimento funzionale necessario per supportare il cliente/utente.

Utilizzando un elemento per indicare la tipologia si forniscono le basi per compiti conosciuti, come \ac{Request-For-Change} o \english{Service Request} o guasti, e permette di avere liste suddivise per categorie come ad esempio:
...
Successivamente ad aver stabilito che il primo elemento della categorizzazione di un \english{ticket} è la tipologia il successivo, la \keyword{categoria}, cambia sulla base della tipologia selezionata. Ad esempio:

\begin{center}
\textsc{richiesta di servizio | aiuto all'utente | desktop application}
\end{center}

Infine viene aggiunto il prodotto oggetto della richiesta, affinché il personale del \english{Service Desk} abbia conoscenza su quale versione del prodotto ricercare le cause dell'anomalia.

La tassonomia \ac{Information-Technology-Infrastructure-Library} aiuta il personale del \english{Service Desk} a sapere a priori quali azioni dovranno essere compiute per ottemperare alla richiesta.