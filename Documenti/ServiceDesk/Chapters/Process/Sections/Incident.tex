...
\hline
Tipo & Specifica la tipologia di \english{ticket}. I valori possono essere ``\english{incident}'' oppure ``\english{service request} & \attribute{lista}\\
\hline
Categoria & Specifica la categoria presso cui si colloca il \english{ticket}. (vedi Sezione \ref{prc-ticket-category} per maggiori dettagli. & \attribute{lista}\\
\hline
Sottocategoria & Specifica la sotto categoria presso cui si colloca il \english{ticket}. (vedi Sezione \ref{prc-ticket-category} per maggiori dettagli. & \attribute{lista}\\
\hline
...
Quando l'utente invia una richiesta di assistenza presso il \english{Service Desk} il \english{software} di supporto (vedi Sezione \ref{sd-tools}) \keyword{crea} un \english{ticket} che ora si trova trova nello stato \attribute{aperto}.
...
Nel momento in cui l'operatore, responsabile del \english{ticket} in oggetto, inizia ad analizzare le possibili cause dell'incidente esso passa nello stato di \attribute{analisi} e cui vi rimane finché una soluzione non viene trovata oppure è necessario eseguire \english{escalation} che porta il \english{ticket} nello stato \attribute{escalation}.

Nel momento in cui una soluzione è disponibile esso passa nello stato di \attribute{risoluzione} finché essa viene applicata e non è confermata, dall'utente che ha aperto la richiesta, come risolutiva.
