...
A seguito di una supervisione dei locali ed aver valutato come sono suddivise le risorse (vedi Sezione \ref{sd-resources}) sui diversi presidi si ritiene corretta l'implementazione di un \keyword{\english{Service Desk} centralizzato}.
...
Considerando che l'intera struttura ospedaliera è collocata all'interno del medesimo comune (seppur di grandi dimensioni), la duplicazione della funzione di \english{Service Desk} su entrambi i presidi è da sconsigliarsi.

Questo perché la maggior parte delle risorse (\english{software} ed \english{hardware}) sono collocate all'interno della sede principale e la duplicazione non porterebbe alcun beneficio ma solo una duplicazione dei costi di gestione.
